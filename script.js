...
const url =
  "https://restcountries.com/v3.1/all?fields=name,capital,region,population,flags,languages,currencies,subregion,tld,translations";
...
lupa.addEventListener("click", () => {
  fetch(url)
    ...
    .then((data) => {
      ...
      for (let i = 0; i < data.length; i++) {
        if (
          data[i].translations.spa.common.toLowerCase() == pais.toLowerCase() ||
          data[i].name.common.toLowerCase() == pais.toLowerCase()
        ) {
  ...
  if (e.key === "Enter") {
    fetch(url)
      ...
      .then((data) => {
        ...
        for (let i = 0; i < data.length; i++) {
          if (
            data[i].translations.spa.common.toLowerCase() ==
              pais.toLowerCase() ||
            data[i].name.common.toLowerCase() == pais.toLowerCase()
          ) {
            let name = data[i].name.nativeName;
